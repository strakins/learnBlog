...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const res = await fetch('/api/comment/getcomments?limit=4');
        const data = await res.json();
        if (res.ok) {
    ...
    <>
       ...
       { currentUser.isAdmin || currentUser.isCreator ? 
            <div className='p-3 md:mx-auto'>
                <section className='flex-wrap flex gap-4 justify-center'>
                    <article className='flex flex-col p-3 dark:bg-slate-800 gap-4 md:w-72 w-full rounded-md shadow-md'>
                    <div className='flex justify-between'>
                        <div className=''>
                        <h3 className='text-color-mode text-md uppercase'>Total Users</h3>
            ...
            'Sorry You Not Have the permission to view this page'
        }
